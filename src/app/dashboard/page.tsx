...
export default async function DashboardPage() {
  ...
  return <main className="min-h-screen bg-[#F5F6FA] text-[#111322]">
    <header className="sticky top-0 z-10 flex h-[76px] items-center justify-between border-b border-[#E7E8EF] bg-white px-5 sm:px-10 lg:px-14">
      ...
      <nav className="hidden items-center gap-7 text-[13px] font-medium text-[#6F7385] md:flex"><Link className="font-bold text-[#5B4BFF]" href="/dashboard">Beranda</Link><Link href="#informasi">Informasi</Link><Link href="/finance/reports">Keuangan</Link><Link href="/reports">Laporan Saya</Link></nav>
    ...
    <div className="mx-auto flex max-w-[1328px] flex-col gap-6 px-5 py-7 sm:px-10 lg:px-14">
      ...
      <section id="informasi" className="grid gap-6 lg:grid-cols-[1fr_330px]">
        <div className="rounded-[18px] bg-white p-6"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Informasi terbaru</h2><span className="text-xs font-bold text-[#5B4BFF]">Lihat semua →</span></div><div className="mt-3 divide-y divide-[#E7E8EF]">{contents.length === 0 && <p className="py-8 text-sm text-[#6F7385]">Belum ada informasi yang dipublikasikan.</p>}{contents.map(content => <article key={content.id} className="flex gap-4 py-4"><div className="flex h-[58px] w-[58px] shrink-0 items-center justify-center rounded-xl bg-[#EFEDFF] text-2xl">▤</div><div><p className="text-[10px] font-bold tracking-[1px] text-[#5B4BFF]">{content.type}</p><h3 className="mt-1 text-sm font-bold text-[#111322]">{content.title}</h3><p className="mt-1 line-clamp-2 text-[11px] leading-5 text-[#6F7385]">{content.body}</p></div></article>)}</div></div>
