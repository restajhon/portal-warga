...
import { programUpdateSchema } from '@/lib/community-schema'
...
export async function GET(request: Request) {
  ...
  const data = await prisma.programUpdate.findMany({ where: manage ? undefined : { status: 'PUBLISHED' }, orderBy: { createdAt: 'desc' } })
...
export async function POST(request: Request) {
  ...
  const data = await prisma.programUpdate.create({ data: { ...parsed.data, authorId: session.user.id } })
  await writeAuditLog({ actorId: session.user.id, action: 'PROGRAM_UPDATE_CREATED', entityType: 'PROGRAM_UPDATE', entityId: data.id })
